  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (productsRes.data.success) setProductCount(productsRes.data.data.length);
      } catch (error) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-card border rounded-2xl p-6 shadow-sm hover:shadow-md transition">
          ...
          <div className="text-3xl font-bold">৳ {paidOrders.reduce((acc, o) => acc + o.totalAmount, 0).toLocaleString()}</div>
        </div>
        <div className="bg-card border rounded-2xl p-6 shadow-sm hover:shadow-md transition">
          <div className="flex items-center justify-between mb-4">
      ...
      <div className="bg-card border rounded-2xl p-6 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 border-2 border-dashed rounded-xl text-muted-foreground">No recent orders found.</div>
        ) : (
              ...
              <tbody>
                {orders.slice(0, 5).map((order) => (
                  <tr key={order._id} className="border-b last:border-0 hover:bg-muted/30">
                    <td className="px-4 py-4 font-medium truncate max-w-[100px]">{order._id.slice(-8)}</td>
                    ...
                    <td className="px-4 py-4">
                      <span className="px-2 py-1 bg-amber-500/10 text-amber-600 rounded text-xs font-semibold capitalize">
                        {order.orderStatus}
                      </span>
                    </td>
